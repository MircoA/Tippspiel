...
$season = mysqli_query($con, "SELECT season.right_tendency AS tendency, season.bonus_points AS points FROM season WHERE id = $season_id");
...
// echo $bonus_points . ' ' . $right_tendency;

//Liste der Spiele die bereits berechnet werden
$games = mysqli_query($con, "SELECT game.* FROM game, matchday, season WHERE game.status = 2 AND game.matchday_id = matchday.id AND matchday.season_id = $season_id");

while ($_games = mysqli_fetch_array($games, MYSQL_ASSOC)) {

	// echo 'test<br>' . $_games[id];
	...
	mysqli_query($con, "UPDATE prediction SET status = 1, points = 0, exact = 0, difference = 0, goal_number = 0, tendency = 0 WHERE game_id = $_games[id]");
