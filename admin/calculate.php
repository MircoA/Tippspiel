...
$season = mysqli_query($con, "SELECT season.right_tendency AS tendency, season.bonus_points AS points FROM matchday, season WHERE matchday.id = $matchday_id AND season.id = matchday.season_id");
...
$games = mysqli_query($con, "SELECT * FROM game WHERE matchday_id = $matchday_id");

while ($_games = mysqli_fetch_array($games, MYSQL_ASSOC)) {

	//Ergebnis vorhanden und nicht berechnet?
	if ($_games[status] == 1) {
	...
	} else if ($_games[status] != 2) { //weder beendet noch berechnet
		$all = false;
	}
...
if ($all == true) {
	mysqli_query($con, "UPDATE matchday SET status = 2 WHERE id = $matchday_id");
}
